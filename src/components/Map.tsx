...
export default function DisasterMap({ onDisasterSelect }: DisasterMapProps) {
  ...
  const disasterCoordinates = useMemo(
    () =>
      disasters
  ...
  useEffect(() => {
    if (!hasFittedBounds.current && disasterCoordinates.length > 0 && mapRef.current) {
      const bounds = disasterCoordinates.reduce(
          (bounds, coord) => {
            return {
          ...
          {
            minLng: disasterCoordinates[0][0],
            maxLng: disasterCoordinates[0][0],
            minLat: disasterCoordinates[0][1],
            maxLat: disasterCoordinates[0][1]
          }
      );
