...
export default function AIAnalysis() {
  ...
  const handleCheckMisinformation = async (reportId: string) => {
    ...
    try {
      ...
      logger.error('Misinformation check failed', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGenerateSituationReport = async (disasterId: string) => {
    ...
    try {
      ...
      logger.error('Situation report generation failed', error);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-4">
        ...
        {reports.slice(0, 3).map(report => (
          <button
            key={report.id}
            onClick={() => handleCheckMisinformation(report.id)}
            className="px-4 py-2 bg-green-500/20 hover:bg-green-500/30 text-green-300 rounded-lg transition-all hover-lift flex items-center space-x-2"
          >
            <Shield className="w-4 h-4" />
            <span>Verify {report.title}</span>
          </button>
        ))}
      </div>
    </div>
