import { Suspense, lazy, useEffect, useState, type ElementType } from 'react';
import { AlertTriangle, Users, Box, Bell, Plus, Bot } from 'lucide-react';
...
const ResourceModal = lazy(() => import('./ResourceModal'));
...
export default function Dashboard() {
  ...
  const loading = useDisasterStore((state) => state.loading);
  const error = useDisasterStore((state) => state.error);
  const fetchInitialData = useDisasterStore((state) => state.fetchInitialData);
  const subscribeToUpdates = useDisasterStore((state) => state.subscribeToUpdates);
  const unsubscribeFromUpdates = useDisasterStore((state) => state.unsubscribeFromUpdates);
  ...
  }, [fetchInitialData, subscribeToUpdates, unsubscribeFromUpdates]);
  ...
  const renderContent = () => {
    switch (activeView) {
      case 'map':
        return <MapView onDisasterSelect={setSelectedDisaster} />;
      case 'globe':
        return (
  ...
  return (
    <div className="min-h-screen bg-animate pt-16">
      ...
      {showAlertModal && (
        <Suspense fallback={null}>
          <AlertModal onClose={() => setShowAlertModal(false)} disasterId={selectedDisaster || undefined} />
        </Suspense>
      )}
